...
--
-- CORRECTIONS APPLIED:
--   Fix 1: CASE WHEN BETWEEN syntax fixed throughout (PYMT_MTHD, ENTRY_MD, CBK_STAT, CBK_WIN_LOSS, CBK_CYCL, RTRVL_STAT, FULFMT_STAT)
--   Fix 2: Missing CLX_FUND INSERT added
--   Fix 3: PLTF_ID no longer overwritten with NTWRK in settlements; PLTF_ID pulled from merchant
--   Fix 4: MARTS tables now include _KEY, MERCHANT_ID, and count columns required by semantic view
--   Fix 5: RETRIEVAL_FULFILLMENT_RATE case fixed ('Closed' not 'CLOSED')
--   Fix 6: Duplicate GRANT removed
...
$$
    CASE
        WHEN MCC = '5411' THEN 0.0035
        WHEN MCC IN ('5541', '5542') THEN 0.0025
        WHEN MCC = '5812' THEN 0.0012
        WHEN MCC = '5814' THEN 0.0008
        WHEN MCC = '5912' THEN 0.0045
        WHEN MCC = '5732' THEN 0.0085
        WHEN MCC = '5200' THEN 0.0055
        WHEN MCC = '5311' THEN 0.0052
        WHEN MCC = '7011' THEN 0.0089
        WHEN MCC = '7538' THEN 0.0040
        WHEN MCC IN ('8011', '8021') THEN 0.0065
        ELSE 0.0050
    END
$$;
...
BEGIN
    ...
    -- ==========================================================================
    -- Generate Authorization Transactions
    -- FIX 1: CASE WHEN BETWEEN syntax corrected for PYMT_MTHD and ENTRY_MD
    -- ==========================================================================
    ...
    txn_slots AS (
        SELECT
            SEQ4() AS slot_id,
            CASE
                WHEN SEQ4() % 24 BETWEEN 11 AND 13 THEN 1.8
                WHEN SEQ4() % 24 BETWEEN 17 AND 20 THEN 2.0
                WHEN SEQ4() % 24 BETWEEN 9 AND 11 THEN 1.3
                WHEN SEQ4() % 24 BETWEEN 14 AND 16 THEN 1.2
                WHEN SEQ4() % 24 BETWEEN 6 AND 8 THEN 0.8
                WHEN SEQ4() % 24 BETWEEN 21 AND 23 THEN 0.7
                ELSE 0.2
            END AS hour_weight,
    ...
    merchants AS (
        SELECT
            ...
            CASE
                WHEN MCC = '5411' THEN 3.0
                WHEN MCC = '5814' THEN 2.5
                WHEN MCC IN ('5541', '5542') THEN 2.0
                WHEN MCC = '5912' THEN 1.5
                WHEN MCC = '5311' THEN 1.3
                WHEN MCC = '5812' THEN 1.2
                WHEN MCC = '5200' THEN 0.8
                WHEN MCC = '5732' THEN 0.5
                WHEN MCC = '7011' THEN 0.3
                ELSE 1.0
    ...
    SELECT
        ...
        -- FIX 1a: Corrected CASE WHEN BETWEEN syntax for PYMT_MTHD
        CASE
        ...
        END AS PYMT_MTHD,
        r.ntwrk AS NTWRK,
        -- FIX 1b: Corrected CASE WHEN BETWEEN syntax for ENTRY_MD
        CASE
    ...
    -- ==========================================================================
    -- Generate Settlement Data (T+1)
    -- FIX 3: PLTF_ID now sourced from merchant table, not overwritten with NTWRK
    -- ==========================================================================
    ...
    WITH approved_auths AS (
        SELECT
            ...
            b.CARD_TYP,
            m.MCC,
            m.PLTF_ID   -- FIX 3: pull PLTF_ID from merchant master
        FROM CLX_AUTH a
    ...
    SELECT
        ...
        MAX(PLTF_ID) AS PLTF_ID,  -- FIX 3: correctly references merchant PLTF_ID
        NTWRK
    FROM approved_auths
    GROUP BY TXN_DT, CARD_BRND, MRCH_KEY, CLNT_ID, MCC, NTWRK, PLTF_ID;  -- FIX 3: PLTF_ID in GROUP BY

    SELECT COUNT(*) INTO :total_settle FROM CLX_SETTLE;

    -- ==========================================================================
    -- Generate Funding Data (T+2)
    -- FIX 2: This section was missing entirely — now fully implemented
    -- ==========================================================================
    ...
    SELECT
        ...
        sb.CLNT_ID,
        sb.MRCH_KEY,
        DATEADD(DAY, 2, sb.BTCH_DT)                                               AS FUNDED_DT,
        sb.BTCH_DT                                                                 AS SETTLE_DT,
        DATEADD(DAY, 1, sb.BTCH_DT)                                               AS EXPCT_DT,
        ROUND(sb.PRCS_NET_AM - (sb.PRCS_NET_AM * 0.001) - (sb.PRCS_NET_AM * 0.002), 2) AS DPST_AM,
        sb.SALES_AM - sb.RFND_AM                                                  AS NET_SALES_AM,
        sb.DSCN_AM                                                                 AS FEES_AM,
        ROUND(sb.PRCS_NET_AM * 0.001, 2)                                          AS CBK_AM,
        ROUND(UNIFORM(-50.00, 50.00, RANDOM()), 2)                                AS ADJ_AM,
        ROUND(sb.PRCS_NET_AM * 0.002, 2)                                          AS RSRV_AM,
        sb.NET_CT                                                                  AS ITEM_CT,
        sb.SALES_CT                                                                AS SALES_CT,
        sb.RFND_CT                                                                 AS RFND_CT,
        CASE WHEN UNIFORM(0,1,RANDOM()) > 0.05 THEN 'Funded' ELSE 'Pending' END  AS PYMT_STAT,
        CASE WHEN UNIFORM(0,1,RANDOM()) > 0.1 THEN 'ACH' ELSE 'Wire' END         AS PYMT_MTHD,
        LPAD(UNIFORM(1000, 9999, RANDOM())::VARCHAR, 4, '0')                      AS DDA_LST4,
        CASE UNIFORM(1, 5, RANDOM())
            WHEN 1 THEN 'Chase Bank'
            WHEN 2 THEN 'Wells Fargo'
            WHEN 3 THEN 'Bank of America'
            WHEN 4 THEN 'PNC Bank'
            ELSE 'US Bank'
        END                                                                        AS BANK_NM,
        'SALES'                                                                    AS TXN_CTGR,
        'STANDARD'                                                                 AS FUND_TYP,
        sb.BTCH_REF                                                               AS BTCH_REF,
        'ACH' || LPAD(UNIFORM(100000000000, 999999999999, RANDOM())::VARCHAR, 15, '0') AS ACH_TRC,
        sb.PLTF_ID
    ...
    -- FIX 1c: Corrected CASE WHEN BETWEEN for CBK_STAT, CBK_WIN_LOSS, CBK_CYCL
    -- ==========================================================================
    INSERT INTO CLX_CBK (
    ...
    SELECT
        ...
        -- FIX 1c: Corrected CASE WHEN BETWEEN syntax
        CASE
            WHEN UNIFORM(1, 100, RANDOM()) BETWEEN 1 AND 15 THEN 'Open'
            WHEN UNIFORM(1, 100, RANDOM()) BETWEEN 16 AND 35 THEN 'Pending'
            WHEN UNIFORM(1, 100, RANDOM()) BETWEEN 36 AND 60 THEN 'Won'
            WHEN UNIFORM(1, 100, RANDOM()) BETWEEN 61 AND 85 THEN 'Lost'
            ELSE 'Closed'
        END AS CBK_STAT,
        CASE
            WHEN UNIFORM(1, 100, RANDOM()) BETWEEN 1 AND 35 THEN 'Won'
            WHEN UNIFORM(1, 100, RANDOM()) BETWEEN 36 AND 85 THEN 'Lost'
            ELSE NULL
        END AS CBK_WIN_LOSS,
        CASE
            WHEN UNIFORM(1, 100, RANDOM()) BETWEEN 1 AND 75 THEN '1st Chargeback'
            WHEN UNIFORM(1, 100, RANDOM()) BETWEEN 76 AND 90 THEN '2nd Chargeback'
            WHEN UNIFORM(1, 100, RANDOM()) BETWEEN 91 AND 97 THEN 'Pre-Arbitration'
            ELSE 'Arbitration'
        END AS CBK_CYCL,
        cr.CBK_RSN_ID,
    ...
    -- ==========================================================================
    -- Generate Retrieval Requests
    -- FIX 1d: Corrected CASE WHEN BETWEEN for RTRVL_STAT and FULFMT_STAT
    -- ==========================================================================
    ...
    SELECT
        ...
        -- FIX 1d: Corrected CASE WHEN BETWEEN syntax
        CASE
...
select
    ...
    auth.mrch_key,                          -- FIX 4: exposed for mart join
    merchants.mrch_id as merchant_id,       -- FIX 4: exposed for semantic view relationship
...
from auth
left join merchants on auth.mrch_key = merchants.mrch_key
left join bins on auth.bin_id = bins.bin_id
left join processors on auth.pltf_id = processors.pltf_id
left join decline_reasons on auth.dcln_rsn_id = decline_reasons.dcln_rsn_id
;
...
select
    ...
    settlements.mrch_key,                       -- FIX 4: exposed for mart join
    merchants.mrch_id as merchant_id,            -- FIX 4: exposed for semantic view relationship
...
from settlements
left join merchants on settlements.mrch_key = merchants.mrch_key
left join processors on settlements.pltf_id = processors.pltf_id
;
...
select
    ...
    deposits.mrch_key,                          -- FIX 4: exposed for mart join
    merchants.mrch_id as merchant_id,            -- FIX 4: exposed for semantic view relationship
    ...
    processors.pltf_nm as processor_name
from deposits
left join merchants on deposits.mrch_key = merchants.mrch_key
left join processors on deposits.pltf_id = processors.pltf_id
;
...
select
    ...
    chargebacks.mrch_key,                       -- FIX 4: exposed for mart join
    merchants.mrch_id as merchant_id,            -- FIX 4: exposed for semantic view relationship
    ...
    processors.pltf_nm as processor_name
from chargebacks
left join merchants on chargebacks.mrch_key = merchants.mrch_key
left join processors on chargebacks.pltf_id = processors.pltf_id
left join chargeback_reasons on chargebacks.cbk_rsn_id = chargeback_reasons.cbk_rsn_id
;
...
select
    ...
    retrievals.mrch_key,                        -- FIX 4: exposed for mart join
    merchants.mrch_id as merchant_id,            -- FIX 4: exposed for semantic view relationship
    ...
    processors.pltf_nm as processor_name
from retrievals
left join merchants on retrievals.mrch_key = merchants.mrch_key
left join processors on retrievals.pltf_id = processors.pltf_id
;
...
select
    ...
    adjustments.mrch_key,                       -- FIX 4: exposed for mart join
    merchants.mrch_id as merchant_id,            -- FIX 4: exposed for semantic view relationship
...
from adjustments
left join merchants on adjustments.mrch_key = merchants.mrch_key
left join processors on adjustments.pltf_id = processors.pltf_id
;

-- ============================================================
-- SECTION 8: Marts Dynamic Tables
-- FIX 4: All mart tables now include _KEY, MERCHANT_ID, and count columns
--         required by the semantic view
-- ============================================================
...
select
    -- FIX 4: Primary key and merchant relationship required by semantic view
    ...
    -- FIX 4: Count fact required by semantic view metric CHARGEBACK_RATE
...
select
    ...
    interchange_fee_amount              AS interchange_amount  -- FIX 4: required by semantic view fact
from enriched
;
...
select
    ...
    -- FIX 4: Count fact required by semantic view metrics
    1                                   AS DISPUTES_COUNT
from enriched
;
...
-- FIX 5: RETRIEVAL_FULFILLMENT_RATE case corrected ('Closed' not 'CLOSED')
-- ============================================================
USE DATABASE COCO_SDLC_HOL;
USE SCHEMA MARTS;
...
metrics:
  ...
  - name: RETRIEVAL_FULFILLMENT_RATE
    ...
    -- FIX 5: Changed 'CLOSED' to 'Closed' to match generated data casing
...
-- FIX 6: Removed duplicate GRANT on PAYMENT_ANALYTICS_AGENT
-- ============================================================
